We will start out the section with defining well-designed SPARQL. Then we will
...
\begin{definition}[Well-designed SPARQL~\cite{perez2009semantics}]
...
\begin{itemize}
	\item We call the pattern tree $T' = ((V',E',r'), (P_n)_{n \in V'})$ a subtree of $T$
if $(V',E',r')$ is a subtree of $T$. 

\item An extension of $\hat{T}'$ of a subtree
$T'$ of $T$ is a subtree $\hat{T}'$ of $T$, so that $T'$ is in turn a subtree of
$\hat{T}'$. A subtree or extension is proper if some node of the bigger tree is
missing in the smaller tree.

\item Given $T$, we write $V(T)$ to denote the set $V$ of vertices.

\item We denote with $pat(T)$ the set $\bigcup_{n\in V(T)} P_n$ and write $vars(T)$ as an
abbreviation for $vars(pat(T))$.

\item Given a node $n \in V(T)$, we define $branch(n) = n^1, \dots, n^k$ with $n^1 = r$
and $n^k = n$ as the unique sequence of nodes from the root $r$ to $n$.

\item For nodes $n,\hat{n} \in V(T)$, so that $\hat{n}$ is the parent of $n$, let\\
$newvars(n) = vars(n) \backslash vars(branch(\hat{n}))$. 

\item A node $n$ is a child
of a PT $T$ if $n\notin V(T)$ and $n$ is the child of some node $n' \in V(T)$.
\end{itemize}
\end{definition}

After thoroughly describing notions of pattern trees we will use later on, we are now ready
...
\begin{proposition}[\cite{letelier2012static,pichler2014containment}]
	\begin{enumerate}	
		\item[]
...
%graph patterns. 
%We will do something similar in our alternate Definition of
...
Lemma~\ref{lemma:altsemwdpt} describes a characterization which decides whether
...
\begin{definition}[Free and existential variables
...
First the decidable cases of Table~\ref{conttable} are proven, i.e., $S_1$ is an arbitrary subset of
...
To visualize the procedure consider Example~\ref{ex:membershipcuppiempty}.
...
pat(r_2) \cup pat(n_2)$ doesn't exist.
...
\begin{definition}[renamed proper extension~\cite{pichler2014containment}]
...
%In a loop enumerate all finite models $M$ and check in it, if $M
...
%But $I,\Sigma \models Q$, i.e. \textbf{co-CQ-UNDER-TGDs} 
%is equivalent to evaluating if the set of FO formulas $I,\Sigma,\neg Q$ is
%unsatisfiable due to
%the semi-decidability of first-order logic. But then the established
%undecidability of the halting problem would be contradicted.
%\end{proof}
