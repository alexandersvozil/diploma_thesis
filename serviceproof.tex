	...
	\begin{enumerate}
		...
		\item For every variable $y$ that appears
			...
			\begin{enumerate}
				...
				\item If $\hat{P}$ is $(\mbox{SERVICE } u \ P_1)$:
					We know that $Q = trans(P_1,ds,u)$ and by induction hypothesis,
					we know that $Q = (T, \lambda,x)$ fulfills the property. Let
					$r_1$ be the root of $T$. $trans(\cdot)$ adds the conjunct
					$LOC(u,ds)$ and $LOC(g,ds)$ to $r_1$. W.l.o.g. assume $u$,$g$
					...
					\begin{enumerate}
						...
						\item Let $x = g$. This means that we are inside a graph
							pattern $(\mbox{GRAPH } g \ DS)$ and thus $P$ is not well designed
							because $P_1$ contains the subpattern $(P' \OPT P'')$.
						\item Let $x = ds$.
							Let $x = g$. This means that we are inside a graph
							pattern $(\mbox{GRAPH } g \ DS)$ and thus $P$ is not well designed
							because $P_1$ contains the subpattern $(P' \OPT P'')$.

					\end{enumerate}

				\item If $P$ is $(P_1 \UNION  P_2)$:
...
\begin{proof}
	...
	\begin{enumerate}
		\item $Q_1(D) \subseteq Q_2(D)$:
			%In this case we can use Theorem~\ref{biglemma} to conclude that $P_1
			%\subseteq P_2$ and we are done.
			%hier noch genauer
		...
		\item $Q_1(D) \not\subseteq Q_2(D)$: By assumption there is a $\mu \in
			...
			\rr^{DS}_{def}$.
			%das problem ist das wir nicht wissen welcher DS das gegenbeispiel
			%liefert.	
			
			%Using
			%Theorem~\ref{biglemma} we can conclude that $\mu \in \ll P_1
			%\rr^{DS}_{def}$ and  $\mu \notin \ll P_2\rr^{DS}_{def}$.
	\end{enumerate}
\end{proof}
